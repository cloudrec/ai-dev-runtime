...
# ── the scope check must read the list the ACTUATOR enforces (2026-09-02) ───
# `_read_canary_allowlist` opened one hardcoded file, `canary.conf`. systemd reads
# every *.conf in the drop-in directory in LEXICAL order and lets the last
# assignment win, and this host has two:
#
#   canary.conf             (Aug 3)  …=cp-canary:0.0
#   zz-actuation-scope.conf (Aug 5)  …=cp-canary:0.0,mess-qa-automation:0.0
#
# `zz-` sorts last, so the live process ran the wider list — confirmed against
# /proc. The report read only the first file, declared the allowlist to be
# cp-canary alone, and counted mess-qa-automation as a BREACH when it had actually
# been GRANTED. A widening applied through a later-sorting drop-in was invisible to
# the one report whose job is to notice it.

def _dropins(tmp_path, files):
    d = tmp_path / "ai-runtime.service.d"
    d.mkdir()
...
def test_the_last_sorting_dropin_wins_like_systemd(tmp_path):
    d = _dropins(tmp_path, {
        "canary.conf": "[Service]\nEnvironment=CONTROL_PLANE_CANARY_AGENTS=cp-canary:0.0\n",
        "zz-actuation-scope.conf":
            "[Service]\nEnvironment=CONTROL_PLANE_CANARY_AGENTS=cp-canary:0.0,mess-qa-automation:0.0\n",
    })
    assert diag._read_canary_allowlist(dropin_dir=d) == {
        "cp-canary:0.0", "mess-qa-automation:0.0"}


def test_a_widening_dropin_is_not_reported_as_a_breach(tmp_path):
    """The consequence that mattered: a granted target must stop counting as an
    escape, or the report cries wolf about the owner's own decision."""
    _cp_action("cp-canary:0.0")
    _cp_action("mess-qa-automation:0.0")
    d = _dropins(tmp_path, {
        "canary.conf": "[Service]\nEnvironment=CONTROL_PLANE_CANARY_AGENTS=cp-canary:0.0\n",
        "zz-scope.conf":
            "[Service]\nEnvironment=CONTROL_PLANE_CANARY_AGENTS=cp-canary:0.0,mess-qa-automation:0.0\n",
    })
    r = diag.actuation_scope_report(dropin_dir=d)
    assert r["unexpected_actuated"] == [] and r["status"] == "green"


def test_a_target_outside_every_dropin_is_still_a_breach(tmp_path):
    """Widening the read must not widen the verdict."""
    _cp_action("cp-canary:0.0")
    _cp_action("arbitrage2-opus:0.0")
    d = _dropins(tmp_path, {
        "canary.conf": "[Service]\nEnvironment=CONTROL_PLANE_CANARY_AGENTS=cp-canary:0.0\n",
        "zz-scope.conf":
            "[Service]\nEnvironment=CONTROL_PLANE_CANARY_AGENTS=cp-canary:0.0,mess-qa-automation:0.0\n",
    ...
    r = diag.actuation_scope_report(dropin_dir=d)
    assert r["unexpected_actuated"] == ["arbitrage2-opus:0.0"]


def test_a_commented_out_assignment_is_ignored(tmp_path):
    """`99-autopilot.conf` mentions the variable inside a comment. Reading that as
    an assignment would silently narrow or widen the allowlist."""
    d = _dropins(tmp_path, {
        "10-real.conf": "[Service]\nEnvironment=CONTROL_PLANE_CANARY_AGENTS=cp-canary:0.0\n",
        "20-note.conf": "# Actuation stays confined to CONTROL_PLANE_CANARY_AGENTS=everything:0.0\n",
    })
    assert diag._read_canary_allowlist(dropin_dir=d) == {"cp-canary:0.0"}


def test_an_explicit_single_path_still_works(tmp_path):
    f = tmp_path / "one.conf"
    f.write_text("[Service]\nEnvironment=CONTROL_PLANE_CANARY_AGENTS=only:0.0\n")
    assert diag._read_canary_allowlist(dropin_path=str(f)) == {"only:0.0"}


def test_no_dropins_falls_back_to_the_process_env(tmp_path, monkeypatch):
    """With nothing on disk the in-process allowlist is the only truth available."""
    from core.control_plane import actuator
    monkeypatch.setattr(actuator, "CANARY_AGENTS", frozenset({"from-env:0.0"}))
    assert diag._read_canary_allowlist(dropin_dir=str(tmp_path / "absent")) == {"from-env:0.0"}


# ── reading the config that is actually IN FORCE (2026-09-02) ──────────────
# The canary allowlist lives in a drop-in; NATIVE_SUPERVISOR_TARGETS lives in the
# unit's EnvironmentFile. A reader that consults only one source misses whichever
# value is in the other, so `effective_service_env` merges both the way systemd
# does: EnvironmentFile first, then drop-ins in lexical order, last wins.

def test_a_dropin_overrides_the_environment_file(tmp_path):
    envf = tmp_path / "env"
    envf.write_text("X_SCOPE=from-env-file\n")
    d = _dropins(tmp_path, {"zz.conf": "[Service]\nEnvironment=X_SCOPE=from-dropin\n"})
    assert diag.effective_service_env("X_SCOPE", env_files=[str(envf)],
                                      dropin_dir=d) == "from-dropin"


def test_the_environment_file_supplies_what_no_dropin_sets(tmp_path):
    envf = tmp_path / "env"
    envf.write_text("X_ONLY=from-env-file\n")
    d = _dropins(tmp_path, {"zz.conf": "[Service]\nEnvironment=OTHER=1\n"})
    assert diag.effective_service_env("X_ONLY", env_files=[str(envf)],
                                      dropin_dir=d) == "from-env-file"


def test_an_unset_variable_returns_the_default(tmp_path):
    d = _dropins(tmp_path, {"a.conf": "[Service]\nEnvironment=OTHER=1\n"})
    assert diag.effective_service_env("MISSING", env_files=[], dropin_dir=d,
                                      default="fallback") == "fallback"


@pytest.mark.parametrize("name", ["RUNTIME_TOKEN", "TELEGRAM_BOT_TOKEN",
                                  "TELEGRAM_CHAT_ID", "some_api_key", "DB_PASSWORD"])
def test_it_refuses_to_read_secrets(name):
    """This exists to make CONFIG auditable. A general "read any variable out of the
    unit files" helper would be a credential-reading tool wearing a diagnostics
    label — and the unit's EnvironmentFile is exactly where the token lives."""
    with pytest.raises(ValueError):
        diag.effective_service_env(name)
